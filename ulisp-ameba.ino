/* uLisp Ameba Version 3.3 - www.ulisp.com
   David Johnson-Davies - www.technoblogy.com - 18th September 2020
...
#define BUFFERSIZE 34  // Number of bits+2
  ...
  #include <strings.h>
  #define WORKSPACESIZE 5760-SDSIZE      /* Objects (8*bytes) */
...
inline void WiFiwrite (char c) { wifiClient.write(c); }
#if defined(sdcardsupport)
...
#if defined(ARDUINO_AMEBA)
  if (!((pin>=0 && pin<=2) || (pin>=18 && pin<=21))) error(ANALOGREAD, invalidpin, number(pin));
#endif
}

void checkanalogwrite (int pin) {
#if defined(ARDUINO_AMEBA)
  if (!((pin>=8 && pin<=13) || (pin>=16 && pin<=18) || (pin>=20 && pin<=21) || (pin>=23 && pin<=24))) error(ANALOGREAD, invalidpin, number(pin));
#endif
}
...
object *sp_withclient (object *args, object *env) {
  ...
  if (params == NULL) {
    ...
    if (!wifiClient) return nil;
    n = 2;
  } else {
    object *address = eval(first(params), env);
    object *port = eval(second(params), env);
    int success;
    if (stringp(address)) success = wifiClient.connect(cstringbuf(address), checkinteger(WITHCLIENT, port));
    else if (integerp(address)) success = wifiClient.connect(address->integer, checkinteger(WITHCLIENT, port));
...
object *fn_available (object *args, object *env) {
  ...
  return number(wifiClient.available());
}
...
object *fn_wifisoftap (object *args, object *env) {
  ...
  object *arg = first(args); args = cdr(args);
  cstring(arg, ssid, 33);
  if (args != NULL) cstring(first(args), pass, 65);
  // Check wifi status
  status = WiFi.status();
  while (status != WL_CONNECTED) {
    if (args == NULL) status = WiFi.apbegin(ssid, channel);
    else status = WiFi.apbegin(ssid, pass, channel);
    delay(5000);
  }
  return lispstring((char*)(toString(WiFi.localIP()).c_str()));
}

object *fn_connected (object *args, object *env) {
  (void) env;
  if (isstream(first(args))>>8 != WIFISTREAM) error2(CONNECTED, PSTR("invalid stream"));
  return wifiClient.connected() ? tee : nil;
}
...
object *fn_wificonnect (object *args, object *env) {
  ...
  else result = WiFi.begin(cstring(first(args), ssid, 33), cstring(second(args), pass, 65));
  if (result == WL_CONNECTED) return lispstring((char*)(toString(WiFi.localIP()).c_str()));
...
int gserial () {
  ...
  while (!KybdAvailable) {
    while (!Serial.available()) callRTOS();
